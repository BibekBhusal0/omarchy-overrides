...
- **herdr session workflow.** `bin/herdr-session-create` / `-search` (via `utils/herdr.sh`) use fzf over open workspaces + zoxide dirs, launching via [herdr-spreader](https://github.com/yuk1ty/herdr-spreader) (`apply --file <dir>/herdr.yml`). A tab must always have at least one pane entry (`- {}` for an empty shell — bare `-` fails to parse). Re-applying is guarded: if a workspace with the layout's `name:` already exists it only switches.
