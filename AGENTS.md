...
- `bin/` — custom CLIs symlinked into `~/.local/my-bins` (on `PATH` via `uwsm/env` and `zsh/shell.sh`): `tmux-session-*`, `focusd-walker`, `toggle-animation`, `mkrepo`, `start`.
- `files_to_copy/` — full config files symlinked wholesale: `waybar/`, elephant menu lua files.
- `utils/` — sourced bash libraries (`clone`, `symlink`, `write-to-file`, `tmux`), guarded by `[[ "${BASH_SOURCE[0]}" == "${0}" ]]` so they no-op when sourced. Source them, don't execute.
