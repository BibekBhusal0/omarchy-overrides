...
- `config/` — git aliases (`config/git.sh`) and omarchy plugin setup (`config/omarchy.sh`: symlinks own shell plugins from `~/Code/omarchy-shell-plugins`, clones third-party plugins into `~/Code/other-omarchy-plugins` and symlinks each into `~/.config/omarchy/plugins`, symlinks `shell.json`).
