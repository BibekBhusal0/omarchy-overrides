...
- `install/`: one-time package/app setup. Uses `omarchy-pkg-aur-add`, `omarchy-install-dev-env`, `omarchy-tui-install`, `omarchy-webapp-install`, `omarchy-install-browser`. Some clone from the owner's GitHub via `utils/clone.sh`, including `install/skills.sh` which clones `~/Code/skills` and symlinks its global instructions and skill folders into the opencode, agents, Claude, and Codex locations.
- `overwrite/`: layers configs onto `$HOME` via `source-overwrites.sh`, which appends `source = ...` lines to hyprland, zsh, bashrc, tmux, ghostty, XCompose. Also creates symlinks for hyprland.lua, hyprsunset, starship, yazi configs.
- `config/`: git aliases (`config/git.sh`) and omarchy plugin setup (`config/omarchy.sh`, which symlinks own shell plugins from `~/Code/omarchy-shell-plugins`, clones third-party plugins into `~/Code/other-omarchy-plugins` and symlinks each into `~/.config/omarchy/plugins`, symlinks `shell.json`).
- `bin/`: custom CLIs symlinked wholesale into `~/.local/my-bins` (on PATH via `uwsm/env` and `zsh/shell.sh`): `tmux-session-*`, `tmux-window-search`, `herdr-session-*`, `herdr-nav`, `cliamp-daemon`, `devmoji`, `mkrepo`, `start`, `clean-omarchy-plugins`, `omarchy-plugin-update` (shadows the stock one via PATH; `zsh/alias.sh` also routes `omarchy plugin update` to it since the stock dispatcher only reads `/usr/share/omarchy/bin`).
...
- **`utils/clone.sh`:** `clone <name> [dest]` where bare names default to `https://github.com/bibekbhusal0/<name>`; supports full URLs and `user/repo` format; skips silently if dest already exists.
